...
error = () => {
  console.log("Error loading network time.\nLoading local time")
...
window.addEventListener("DOMContentLoaded", function() {
  ...
  request.open("GET", "http://worldtimeapi.org/api/timezone/Europe/Paris", !0);
  ...
  request.onload = function() {
    if (200 <= request.status && 400 > request.status) {
      ...
      var h = g.datetime.split("T")[0].split("-");
      nowMonth = thisMonth = new RightTime(h[0],h[1],h[2]);
      generateMonth();
      $("noscript").style.display = "none";
    } else {
      error()
